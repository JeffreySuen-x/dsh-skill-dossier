...
function apply(ctx) {
	...
	ctx.effect(() => () => {
		active = false;
		for (const registrations of owned.values()) for (const dispose of registrations.values()) dispose();
		owned.clear();
	});
	...
	function isOwned(sessionId, name) {
		...
		return agent !== void 0 && owned.get(agent)?.has(name) === true;
	}
	...
	const handlers = {
		...
		async register(args) {
			...
			return enqueueTemporary(agent, async () => {
				...
				const existing = (await skills.list({
					scope: agent,
					cwd: agent.session.header.cwd
				})).some((s) => s.name === name);
				if (!active || agentOf(sessionId) !== agent || owned.get(agent) !== registrations) return {
					ok: false,
					error: "插件或当前会话已停止"
				};
				if (existing && !registrations.has(name)) return {
					ok: false,
					error: `同名技能 "${name}" 已存在`
				};
