...
describe('scoreSkill', () => {
  it('boosts a skill whose direction matches the query keywords over an unrelated one', () => {
    const coding = profile('coding-qa', '工程代码', '编码任务质量闸门')
    const content = profile('aeon-content', '内容写作', '视频脚本')
  ...
  it('prefers a research skill over a frontend skill for a 调研 query', () => {
    const research = profile('research', '调研报告', '调研问题并抓取一手来源')
  ...
  it('bonuses a skill-name token present in the query', () => {
    const result = scoreSkill('帮我 review 一下代码', profile('code-review', '工程代码', '审查变更'))
    expect(result.matched).toContain('方向:工程代码')
  })
})
...
describe('matchSkills', () => {
  it('returns top-K sorted by score and honours the direction filter', () => {
    const profiles = [
      profile('research', '调研报告', '调研问题'),
      profile('china-industry-research', '调研报告', '行业深度调研报告'),
      profile('ponytail', '工程代码', '写代码最简实现'),
    ]
    const all = matchSkills('做一份行业调研报告', profiles, { topK: 10 })
    expect(all[0]?.name).toBe('china-industry-research')
    const onlyEngineering = matchSkills('做一份行业调研报告', profiles, { topK: 10, direction: '工程代码' })
  ...
  it('truncates long profile fields in the result', () => {
    ...
    const [match] = matchSkills('任意', [profile('x', '工程代码', long)], { topK: 1 })
...
describe('formatMatches', () => {
  ...
  it('lists matches one per line with direction and score', () => {
    const text = formatMatches(
      [scoreSkill('写代码', profile('ponytail', '工程代码', '最简实现'))],
      1,
      '写代码',
    )
    expect(text).toContain('ponytail')
    expect(text).toContain('工程代码')
