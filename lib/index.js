...
function moveNoClobberCommand(src, dst, isWindows) {
	...
	if (isWindows) return `if ([System.IO.Directory]::Exists(${source})) { [System.IO.Directory]::Move(${source}, ${destination}) } elseif ([System.IO.File]::Exists(${source})) { if (([System.IO.File]::GetAttributes(${source}) -band [System.IO.FileAttributes]::ReparsePoint) -ne 0) { throw 'file links are not supported' }; [System.IO.File]::CreateHardLink(${destination}, ${source}) | Out-Null; try { [System.IO.File]::Delete(${source}) } catch { [System.IO.File]::Delete(${destination}); throw } } else { throw 'source missing' }`;
