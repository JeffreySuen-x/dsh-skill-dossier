...
function moveNoClobberCommand(src, dst, isWindows) {
	...
	return isWindows ? `if (Test-Path -LiteralPath ${destination}) { throw 'destination exists' }; Move-Item -LiteralPath ${source} -Destination ${destination} -ErrorAction Stop` : `if [ -e ${destination} ] || [ -L ${destination} ]; then echo 'destination exists' >&2; exit 17; fi; mv -n -- ${source} ${destination}; [ ! -e ${source} ] && [ ! -L ${source} ]`;
}
...
function apply(ctx) {
	...
	async function rollbackMoveOrRethrow(operationError, source, destination, root) {
		try {
	...
	const handlers = {
		...
		async uninstall(args) {
			...
			try {
				...
				if (moved) return rollbackMoveOrRethrow(error, trashedPath, entryInfo.entry, entryInfo.root);
				throw error;
			}
			return { ok: true };
		},
		async reinstall(args) {
			...
			try {
				...
				if (rollback !== void 0) return rollbackMoveOrRethrow(error, rollback.source, rollback.destination, rollback.root);
				throw error;
			}
		},
