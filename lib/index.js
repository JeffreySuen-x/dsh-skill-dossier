...
function toMarkdown(data, view) {
	...
	if (view === "monthly") {
		...
		for (const day of data.days ?? []) lines.push(`| ${String(day.date ?? "")} | ${(day.projects ?? []).join("、")} | ${(day.progress ?? []).join("；")} | ${(day.todo ?? []).join("；")} | ${(day.issues ?? []).join("；")} |`);
		lines.push("", "## 跨日项目汇总");
...
function apply(ctx) {
	...
	function normalizeIndex(parsed) {
		...
		return {
			...
			usage: p.usage !== null && typeof p.usage === "object" ? p.usage : {}
		};
	}
	...
	* 异步埋点与显式建档/停用操作相互覆盖。 */
	const indexWrites = /* @__PURE__ */ new Map();
	function withIndexLock(cwd, task) {
		const next = (indexWrites.get(cwd) ?? Promise.resolve()).then(task, task);
		indexWrites.set(cwd, next.then(() => void 0, () => void 0));
		return next;
	}
	/** 记录一次技能调用：读取 index → 折叠进 usage → 回写。 */
	function recordSkillUse(cwd, name) {
		if (!NAME_RE.test(name)) return;
		withIndexLock(cwd, async () => {
