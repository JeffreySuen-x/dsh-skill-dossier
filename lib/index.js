...
function posixIdentityCommand(target) {
...
function moveNoClobberCommand(src, dst, isWindows) {
	...
	if (isWindows) return `if ([System.IO.Directory]::Exists(${source})) { [System.IO.Directory]::Move(${source}, ${destination}) } elseif ([System.IO.File]::Exists(${source})) { [System.IO.File]::Move(${source}, ${destination}) } else { throw 'source missing' }`;
